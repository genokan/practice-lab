...
UNIT
  ...
  sudo -n tee "$k3s_ingress_service_path" >/dev/null <<UNIT
[Unit]
Description=Proxy practice-lab k3s ingress through MB1
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/bin/ncat --listen --keep-open ${mb1_lan_ip} 8080 --sh-exec '/usr/bin/ncat ${control_plane_ip} 80'
Restart=on-failure
RestartSec=2
